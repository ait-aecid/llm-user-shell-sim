# experiments/tfidf_360_nested.py
#
# Run:
...
# - Inner loop: for each outer split:
#     - loop over a small LoadConfig grid
#     - run tfidf.pipeline.search() over candidate grid
#     - pick best (LoadConfig + Candidate) by VAL f1_macro ONLY
...
from core.loader import load_examples, LoadConfig, get_num_actor_label_assignments
from core.splits import make_splits
from core.val_test_combs import make_val_test_splits
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="Data",
        choices=["Data", "Data_WP"],
        help="Which aggregated dataset root to use.",
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Logical log source to evaluate. "
             "For dataset=Data: audit, syslog, nextcloud. "
             "For dataset=Data_WP: audit, syslog.",
    )
