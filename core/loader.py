...
def _default_data_root(dataset: DatasetName) -> Path:
    return PROJECT_ROOT / dataset / "combine" / "ExperimentAggregated"

def get_num_actor_label_assignments(dataset: DatasetName) -> int:
...
def _default_groups(dataset: DatasetName) -> Tuple[Tuple[str, ...], Tuple[str, ...]]:
    if dataset == "Data":
        return (
